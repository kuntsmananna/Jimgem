...
-- The shared to-do list (scripts/migrate-029-tasks.sql). This declaration
-- and its migration's are repeated word for word, both IF NOT EXISTS, so
-- whichever ran first wins and the other is a no-op -- keep them
-- identical, for the reason row_revisions states above
